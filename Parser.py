...
class Parser:
    def __init__(self, _filename_=None):
        self.root = None
        self.lexer = Lexer()
        self.lexer.readFile(_filename_)
        self.lexer.conversionText()
        self.buffer = [None, None]

    def get_next_token(self):
        self.buffer[0] = self.buffer[1]
        self.buffer[1] = self.lexer.lexer()
        # print(str(self.buffer[0].token_class) + " " + str(self.buffer[1].token_class))

    def handle_arrays(self, name):
        self.get_next_token()
        self.get_next_token()
        index = None
        if self.buffer[0].lexeme == "]":
            ...
            if not (self.buffer[1].token_class in priority):
                if self.buffer[0].token_class == "Num" or self.buffer[0].token_class == "Real_Num" or \
                        self.buffer[0].token_class == "Num_Bin" or self.buffer[0].token_class == "Num_Oct" or \
                    ...
                    self.get_next_token()
                    if self.buffer[1].token_class == "Exponentiation":
                        index = AST.UnaryExpressionAST(op, None, self.handle_arithmetical_expression())
                    else:
                        if self.buffer[0].token_class == "Num" or self.buffer[0].token_class == "Real_Num" or \
                                self.buffer[0].token_class == "Num_Bin" or self.buffer[0].token_class == "Num_Oct" or \
                                self.buffer[0].token_class == "Num_Hex":
                            index = AST.UnaryExpressionAST(op, None,
                                                           AST.NumberExpressionAST(int(self.buffer[0].lexeme), None))
                        elif self.buffer[1] and self.buffer[0].token_class == "Id" and \
                                self.buffer[1].token_class == "L_Paren_Bracket":
                            index = AST.UnaryExpressionAST(op, None, self.handle_call_function(self.buffer[0].lexeme))
                        elif self.buffer[1] and self.buffer[0].token_class == "Id" and \
                                self.buffer[1].token_class == "L_Sq_Bracket":
                            index = AST.UnaryExpressionAST(op, None, self.handle_arrays(self.buffer[0].lexeme))
                        elif self.buffer[0].token_class == "Id":
                            index = AST.UnaryExpressionAST(op, None,
                                                           AST.VariableExpressionAST(self.buffer[0].lexeme, None))
                elif self.buffer[0].token_class == "L_Sq_Bracket":
                    index = self.handle_create_list()
            else:
        ...
        while self.buffer[0].lexeme != ")":
            if not (self.buffer[1].token_class in priority):
                if self.buffer[0].token_class == "Num" or self.buffer[0].token_class == "Real_Num" or \
                    ...
                    self.get_next_token()
                    if self.buffer[1].token_class == "Exponentiation":
                        args.append(AST.UnaryExpressionAST(op, None, self.handle_arithmetical_expression()))
                    else:
                        if self.buffer[0].token_class == "Num" or self.buffer[0].token_class == "Real_Num" or \
                                self.buffer[0].token_class == "Num_Bin" or self.buffer[0].token_class == "Num_Oct" or \
                                self.buffer[0].token_class == "Num_Hex":
                            args.append(AST.UnaryExpressionAST(op, None,
                                                               AST.NumberExpressionAST(int(self.buffer[0].lexeme),
                                                                                       None)))
                        elif self.buffer[1] and self.buffer[0].token_class == "Id" and \
                                self.buffer[1].token_class == "L_Paren_Bracket":
                            args.append(
                                AST.UnaryExpressionAST(op, None, self.handle_call_function(self.buffer[0].lexeme)))
                        elif self.buffer[1] and self.buffer[0].token_class == "Id" and \
                                self.buffer[1].token_class == "L_Sq_Bracket":
                            args.append(AST.UnaryExpressionAST(op, None, self.handle_arrays(self.buffer[0].lexeme)))
                        elif self.buffer[0].token_class == "Id":
                            args.append(AST.UnaryExpressionAST(op, None,
                                                               AST.VariableExpressionAST(self.buffer[0].lexeme, None)))
                elif self.buffer[0].token_class == "L_Sq_Bracket":
                    args.append(self.handle_create_list())
            else:
                if self.buffer[0].token_class == "Comma":
                    self.get_next_token()
                args.append(self.handle_arithmetical_expression())
    ...
    def handle_arithmetical_expression(self):
        ...
        if self.buffer[0].token_class == "Num" or self.buffer[0].token_class == "Real_Num" or \
                self.buffer[0].token_class == "Num_Bin" or self.buffer[0].token_class == "Num_Oct" or self.buffer[
            ...
            left = self.handle_arrays(self.buffer[0].lexeme)
        elif self.buffer[0].token_class == "Id":
            ...
            self.get_next_token()
            if self.buffer[1].token_class == "Exponentiation":
                left = AST.UnaryExpressionAST(op, None, self.handle_arithmetical_expression())
            else:
                if self.buffer[0].token_class == "Num" or self.buffer[0].token_class == "Real_Num" or \
                        self.buffer[0].token_class == "Num_Bin" or self.buffer[0].token_class == "Num_Oct" or \
                        self.buffer[0].token_class == "Num_Hex":
                    left = AST.UnaryExpressionAST(op, None, AST.NumberExpressionAST(int(self.buffer[0].lexeme), None))
                elif self.buffer[1] and self.buffer[0].token_class == "Id" and \
                        self.buffer[1].token_class == "L_Paren_Bracket":
                    left = AST.UnaryExpressionAST(op, None, self.handle_call_function(self.buffer[0].lexeme))
                elif self.buffer[1] and self.buffer[0].token_class == "Id" and \
                        self.buffer[1].token_class == "L_Sq_Bracket":
                    left = AST.UnaryExpressionAST(op, None, self.handle_arrays(self.buffer[0].lexeme))
                elif self.buffer[0].token_class == "Id":
                    left = AST.UnaryExpressionAST(op, None, AST.VariableExpressionAST(self.buffer[0].lexeme, None))

        elif self.buffer[0].token_class == "L_Sq_Bracket":
            left = self.handle_create_list()
            if left is None:
                self.get_next_token()
        ...
        while self.buffer[1].token_class in priority:
            ...
            if self.buffer[0].token_class == "Num" or self.buffer[0].token_class == "Real_Num" or \
                ...
                right = self.handle_arrays(self.buffer[0].lexeme)
                right.parent = node
            elif self.buffer[0].token_class == "Id":
                ...
                self.get_next_token()
                if self.buffer[1].token_class == "Exponentiation":
                    right = AST.UnaryExpressionAST(op, None, self.handle_arithmetical_expression())
                else:
                    if self.buffer[0].token_class == "Num" or self.buffer[0].token_class == "Real_Num" or \
                            self.buffer[0].token_class == "Num_Bin" or self.buffer[0].token_class == "Num_Oct" or \
                            self.buffer[0].token_class == "Num_Hex":
                        right = AST.UnaryExpressionAST(op, None,
                                                       AST.NumberExpressionAST(int(self.buffer[0].lexeme), None))
                    elif self.buffer[1] and self.buffer[0].token_class == "Id" and \
                            self.buffer[1].token_class == "L_Paren_Bracket":
                        right = AST.UnaryExpressionAST(op, None, self.handle_call_function(self.buffer[0].lexeme))
                    elif self.buffer[1] and self.buffer[0].token_class == "Id" and \
                            self.buffer[1].token_class == "L_Sq_Bracket":
                        right = AST.UnaryExpressionAST(op, None, self.handle_arrays(self.buffer[0].lexeme))
                    elif self.buffer[0].token_class == "Id":
                        right = AST.UnaryExpressionAST(op, None, AST.VariableExpressionAST(self.buffer[0].lexeme, None))
                self.get_next_token()
    ...
    def handle_if(self, shift):
        ...
        if self.buffer[0].token_class == "Colon":
            ...
            # self.get_next_token()
            if self.buffer[1].token_class == "Keyword_ELSE":
                self.get_next_token()
                self.get_next_token()
    ...
    def handle_for(self, shift):
        ...
        self.get_next_token()
        if self.buffer[0].token_class == "Keyword_IN":
            self.get_next_token()
    ...
    def handle_body(self, shift):
        ...
        while self.buffer[1] and self.buffer[0]:
            if self.buffer[0].token_class == "Keyword_RETURN":
                self.get_next_token()
                body.nodes.append(AST.ReturnAST(self.handle_arithmetical_expression()))
            elif self.buffer[0].token_class == "Keyword_BREAK":
                body.nodes.append(AST.BreakAST())
            elif self.buffer[0].token_class == "Keyword_CONTINUE":
                body.nodes.append(AST.ContinueAST())
            elif (self.buffer[0].token_class == "Num" or self.buffer[0].token_class == "Id" or
                  self.buffer[0].token_class == "RealNum" or self.buffer[0].token_class == "Num_Bin" or self.buffer[0].token_class == "Num_Oct" or
                  self.buffer[0].token_class == "Num_Hex") \
    ...
    def handle_shifts(self):
        shift = 0
        while self.buffer[1].token_class == "Tabulation":
            shift += 1
            self.get_next_token()
        return shift
