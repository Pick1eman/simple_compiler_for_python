...
def get_type_word(type):
    if type == var_type_num:
        return "DWORD PTR"
    elif type == var_type_string:
        return "BYTE PTR"
...
class Symbols_Table:
    def __init__(self, parent=None):
        self.parent = parent
        self.obj_list = []  # Список дочерних нодов
        self.var_list = []  # Список переменных в текущем ноде
...
class ExpressionAST(AST):
    ...
    def check_types(self, curr_node):
        pass
...
class NumberExpressionAST(ExpressionAST):
    ...
    def check_types(self, curr_node=None):
        return var_type_num
...
class BinaryExpressionAST(ExpressionAST):
    ...
    def check_types(self, curr_node):
        left = self.first.check_types(curr_node)
        right = self.second.check_types(curr_node)
        if left == right:
            return left
        else:
            return unknown_type
    ...
    def make_assembler(self):
        ...
        if neg_state == 0:
            code_assembler += "mov eax, " + first_str + "\n"
        if cdq == "cdq":
            code_assembler += cdq + "\n"
    ...
    def check_types(self, curr_node):
        ...
        for i in range(len(self.first)):
            if type(self.first[i]) != type(self.first[i - 1]):
                ...
                return self.first[0].check_types(curr_node)
...
class UnaryExpressionAST(ExpressionAST):
    ...
    def check_types(self, curr_node):
        return self.first.check_types(curr_node)
...
class IfExpressionAST(AST):
    ...
    def find_declaration(self, curr_node, error):
        new_node = Symbols_Table(curr_node)
        if new_node:
            curr_node.obj_list.append(new_node)
        type_ = self.condition.check_types(curr_node)
    ...
    def make_assembler(self):
        ...
        if self.condition.first.get_type_node() == ast_node_types["number_ast"]:
            ...
            code_assembler += get_type_mov(self.condition.first.st_data[1]) + " edx, " + get_type_word(self.condition.first.st_data[1]) + " [rbp-" + str(self.condition.first.st_data[2]) + "]\n"
            ...
            if self.condition.first.first.get_type_node() == ast_node_types["number_ast"]:
                code_assembler += get_type_mov(self.condition.first.first.st_data[1]) + " eax, " + get_type_word(self.condition.first.first.st_data[1]) + " [rbp-" + str(self.condition.first.st_data[2] -
                                                                   self.condition.first.first.value * bytes_shift[self.condition.first.first]) + "]\n"
                ...
                code_assembler += get_type_mov(self.condition.first.first.st_data[1]) + \
                 " eax, " + get_type_word(self.condition.first.first.st_data[1]) + " [rbp-" + str(self.condition.first.first.st_data[2]) + "]\n"
                code_assembler += "cdqe\n"
                code_assembler += get_type_mov(self.condition.first.st_data[1]) + " eax, " + get_type_word(self.condition.first.st_data[1]) + " [rbp-" + str(self.condition.first.st_data[2]) + "+rax*" + str(self.condition.first.st_data[1]) + "]\n"
            code_assembler += "mov edx, eax\n"
        else:
            return
        ...
        if self.condition.second.get_type_node() == ast_node_types["number_ast"]:
            ...
            code_assembler += get_type_mov(self.condition.second.st_data[1]) + " eax, " + get_type_word(self.condition.second.st_data[1]) + " [rbp-" + str(self.condition.second.st_data[2]) + "]\n"
            ...
            if self.condition.second.first.get_type_node() == ast_node_types["number_ast"]:
                ...
                code_assembler += get_type_mov(self.condition.second.first.st_data[1]) + " eax, " + get_type_word(self.condition.second.first.st_data[1]) + " [rbp-" + str(self.condition.second.first.st_data[2]) + "]\n"
        elif self.condition.second.get_type_node() == ast_node_types["array_ast"]:
            if self.condition.second.first.get_type_node() == ast_node_types["number_ast"]:
                code_assembler += get_type_mov(self.condition.second.st_data[1]) + " eax, " + get_type_word(self.condition.second.st_data[1]) + " [rbp-" + str(self.condition.second.st_data[2] -
                                                                   self.condition.second.first.value * 4) + "]\n"
            elif self.condition.second.first.get_type_node() == ast_node_types["variable_ast"]:
                code_assembler += "mov eax, " + get_type_word(self.condition.second.first.st_data[1]) + " [rbp-" + str(self.condition.second.first.st_data[2]) + "]\n"
                code_assembler += "cdqe\n"
                code_assembler += get_type_mov(self.condition.second.st_data[1]) + " eax, " + get_type_word(self.condition.second.st_data[1]) + " [rbp-" + str(self.condition.second.st_data[2]) + "+rax*" + str(self.condition.second.st_data[1]) + "]\n"
        else:
            return
...
class ForExpressionAST(AST):
    ...
    def find_declaration(self, curr_node, error):
        ...
        if found is False:
            new_node.var_list.append([self.init.name, var_type_num])
        type_ = self.condition.check_types(curr_node)
        if type_ == unknown_type:
            print("Некорректное условние в for")
            exit()
            error[0] = True
        self.body.find_declaration(new_node, error)
...
class ElifExpressionAST(AST):
    ...
    def find_declaration(self, curr_node, error):
        ...
        type_ = self.condition.check_types(curr_node)
        if type_ == unknown_type:
            print("Некорректное условние в elif")
            exit()
            error[0] = True
        self.body.find_declaration(new_node, error)
...
class WhileExpressionAST(AST):
    ...
    def find_declaration(self, curr_node, error):
        ...
        if type_ == unknown_type:
            print("Некорректное условние в while")
            exit()
            error[0] = True
        self.body.find_declaration(new_node, error)
    ...
    def make_assembler(self):
        ...
        if self.condition.operation == "and":
            ...
            if self.condition.first.first.get_type_node() == ast_node_types["number_ast"]:
                ...
                operand_ll = get_type_word(self.condition.first.first.st_data[1]) + " [rbp-" + str(self.condition.first.first.st_data[2]) + "]"
            else:
                return
            operand_lr = ""
            if self.condition.first.second.get_type_node() == ast_node_types["number_ast"]:
                ...
                operand_lr = get_type_word(self.condition.first.second.st_data[1]) + " [rbp-" + str(self.condition.first.second.st_data[2]) + "]"
            else:
                return
            operand_rl = ""
            if self.condition.second.first.get_type_node() == ast_node_types["number_ast"]:
                ...
                operand_rl = get_type_word(self.condition.second.first.st_data[1]) + " [rbp-" + str(self.condition.second.first.st_data[2]) + "]"
            else:
                return
            operand_rr = ""
            if self.condition.second.second.get_type_node() == ast_node_types["number_ast"]:
                ...
                operand_rr = get_type_word(self.condition.second.second.st_data[1]) + " [rbp-" + str(self.condition.second.second.st_data[2]) + "]"
            else:
                return
        ...
        if self.condition.first.get_type_node() == ast_node_types["number_ast"]:
            ...
            if self.condition.first.first.get_type_node() == ast_node_types["number_ast"]:
                ...
                code_assembler += get_type_mov(self.condition.first.first.st_data[1]) + " edx, " + get_type_word(self.condition.first.first.st_data[1]) + " [rbp-" + str(self.condition.first.first.st_data[2]) + "]\n"
                code_assembler += "neg edx\n"
        elif self.condition.first.get_type_node() == ast_node_types["array_ast"]:
            if self.condition.first.first.get_type_node() == ast_node_types["number_ast"]:
                code_assembler += get_type_mov(self.condition.first.st_data[1]) + " eax, " + get_type_word(self.condition.first.st_data[1]) + " [rbp-" + str(self.condition.first.st_data[2] -
                                                                   self.condition.first.first.value * bytes_shift[self.condition.first.check_types()]) + "]\n"
            elif self.condition.first.first.get_type_node() == ast_node_types["variable_ast"]:
                code_assembler += "mov eax, DWORD PTR [rbp-" + str(self.condition.first.first.st_data[2]) + "]\n"
                code_assembler += "cdqe\n"
                code_assembler += get_type_mov(self.condition.first.st_data[1]) + " eax, " + get_type_word(self.condition.first.st_data[1]) + " [rbp-" + str(self.condition.first.st_data[2]) + "+rax*" + str(bytes_shift[self.condition.first.check_types()]) + "]\n"
            code_assembler += "mov edx, eax\n"
        else:
            return
        ...
        if self.condition.second.get_type_node() == ast_node_types["number_ast"]:
            ...
            if self.condition.second.first.get_type_node() == ast_node_types["number_ast"]:
                ...
                code_assembler += "mov eax, DWORD PTR [rbp-" + str(self.condition.second.first.st_data[2]) + "]\n"
                code_assembler += "cdqe\n"
                code_assembler += get_type_mov(self.condition.second.st_data[1]) + " eax, " + get_type_word(self.condition.second.st_data[1]) + " [rbp-" + str(self.condition.second.st_data[2]) + "+rax*" + str(self.condition.second.check_types()) + "]\n"
        else:
            return
        code_assembler += "cmp edx, eax\n"
...
class CallFunctionExpressionAST(ExpressionAST):
    ...
    def find_declaration(self, curr_node, error):
        self.check_types(curr_node)

    def check_types(self, curr_node):
        for arg in self.args:
            type_ = arg.check_types(curr_node)
            if type_ == unknown_type:
                print("Неправильный тип переменной в вызове функциии")
                exit()
    ...
    def make_assembler(self):
        global code_assembler
        if self.name == "print" and self.args[0].get_type_node() == ast_node_types["variable_ast"] and len(self.args) == 1:
...
class ArrayExpressionAST(ExpressionAST):
    ...
    def check_types(self, curr_node):
        self.first.check_types(curr_node)
        iter_node = curr_node
        found = unknown_type
        while iter_node:
            for i in range(len(iter_node.var_list)):
...
class AssignmentExpressionAST(ExpressionAST):
    ...
    def find_declaration(self, curr_node, error):
        ...
        type_ = self.second.check_types(curr_node)
        if found is False:
            global last_shift
            if self.second.get_type_node() == ast_node_types["number_ast"] or self.second.get_type_node() == \
                ...
                last_shift += bytes_shift[type_]
            elif self.second.get_type_node() == ast_node_types["init_array_ast"]:
                last_shift += bytes_shift[type_] * len(self.second.first)
            ...
            if prev_type != type_:
                print("Ошибка типов в присвоении переменной '" + self.first.name + "'")
                exit()
                error[0] = True
        if type_ == unknown_type:
            print("Несовместимый тип")
            error[0] = True
        ...
        if self.first.get_type_node() == ast_node_types["variable_ast"]:  # a
            if self.second.get_type_node() == ast_node_types["number_ast"]:  # a = 2
                ...
                for i in range(len(self.second.str_)):
                    code_assembler += "mov BYTE PTR [rbp-" + str(self.first.st_data[2] - i) + "], '" + str(self.second.str_[i]) + "'\n"
